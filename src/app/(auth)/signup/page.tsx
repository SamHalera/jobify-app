...
import { BriefcaseIcon, MailIcon, LockIcon, UserIcon, AlertCircleIcon } from "lucide-react";
...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const { error: authError } = await signUp.email({ name, email, password, callbackURL: "/applications" });
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-1">Créez votre espace de recherche</p>
      </div>
      ...
      <p className="text-center text-sm text-muted-foreground mt-6">
        Déjà un compte ?{" "}
        <Link href="/login" className="font-medium text-[#4361EE] hover:underline">
          Se connecter
        </Link>
      </p>
    </div>
